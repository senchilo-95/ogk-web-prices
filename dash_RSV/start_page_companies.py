...
day_prices = dbc.Card([dcc.Graph(id='my-graph1-ogk')])
hour_prices = dbc.Card([dcc.Graph(id='my-graph2-ogk'),dbc.CardBody(slider)])
date_for_table_1 = df_st_m.index[-1]
date_for_table_2 = df_st_m.index[-2]
table =dbc.Card([dcc.Graph(id='my-table-ogk',config={'displayModeBar': True,'scrollZoom':False,'staticPlot':True})])
collapse = html.Div(
    [
        dbc.Button(
            "Изменение цены",
